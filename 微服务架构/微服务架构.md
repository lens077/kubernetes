...
| Meilisearch（2026-09-04 完整退役） | Elasticsearch |
...
- 现状（2026-09-04）：运行时已切流到 Elasticsearch（投影表 + trigger、publication、topic、Sink 映射、alias 就位，改价 / 删除 / 还原实测约 6 秒生效）；Meilisearch 的 Helm release、运行资源、Secret、路由、PVC/PV 与 namespace 已完整退役。曾经的 outbox → relay → NATS → indexer 增量链因生产者从未存在而空转，已随分线决策删除
