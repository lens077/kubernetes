...
# xt_socket https://docs.cilium.io/en/stable/operations/system_requirements/#admin-system-reqs
# xt_socket 内核模块缺少转发 重定向的 L7 流量在非隧道数据路径中不起作用 模式。
# 由于一些著名的内核（例如 COS）在发布时没有 xt_socket 模块中，Cilium 实现了回退兼容模式，以允许将 L7 策略和可见性与这些内核一起使用。
# 目前，此回退会在非隧道数据路径模式下禁用 ip_early_demux 内核功能，这可能会降低系统网络性能。
# 这保证了 HTTP 和 Kafka 重定向按预期工作。
# 但是，如果从未使用 HTTP 或 Kafka 强制策略，则可以通过向 helm 配置命令行添加以下内容来关闭此行为：
...
#L7 代理重定向目前使用 TPROXY iptables 作以及套接字匹配。要使 L7 重定向按预期工作，内核配置必须包括以下模块：
