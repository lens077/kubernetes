...
- [x] **Web operator role 浏览器级 E2E 跑通 + 顺带抓到 legacy token 命中源**(09-15):control-tower 真环境 Playwright 15/16 绿,含 `/tokens` 签发→WatchKeys 长流→吊销后拒绝读取。唯一红的是 `machine_token_legacy_hits=12`——12 次全是 09-12 我在 Mac 上跑 cart `make dev` 打的:本地 gitignored `source.dev.yaml` 里塞的是 legacy 共享 token,因为 Config Center 的 dev service token 是 8-31 旧内网集群签的(老命名 `cart-service`/`search-product`/`user-identity`,明文已丢)且集群没有 `ecommerce-config-source-dev`。修法:harvest 补「selector 里没它就签」(原先配置无差异会跳过,新环境永远建不出 selector),`ENV=dev harvest` 建 selector + 10 枚 dev token,Mac 本地文件切到 `ct_` token(`make dev` 复跑 legacy 命中 12→12 不增),8-31 那 10 枚孤儿已吊销。**处理那条红的(09-15)**:① 重启 config-center 归零当前值(量表从进程启动累计,不重启永不归零——这是让它有归零之日的必要动作,不是作弊);② 7d 窗口看的是样本不是命中:旧进程一直上报 12 到 07:50Z 死掉,所以 `max_over_time[7d]` 要到 **09-22 07:50Z** 才归零,比文档预想的 09-19 晚;③ 不删 VM 历史(那是篡改审计)。**真正的解决是关掉 legacy 分支**:集群全部 Secret 核对无任何消费者再持有共享 token,control-tower 分支 `chore/retire-legacy-shared-token`(3bc06e5)已删 `CONFIG_CENTER_SERVICE_TOKEN` 环境变量/Secret 引用/`MachineScope.Legacy`/量表/e2e 零命中用例,Go 全绿;按规则最后一次命中 + 7d = **09-19 03:55Z 后合并发版**。顺手修了 main 上过时的 `TestConfigImagesUseGHCR`→TCR(094ba3d);**注意**:TCR 仓库私有(匿名 401),而 058a97e 换了 TCR 镜像却没加 imagePullSecrets、另一条测试还断言不得引用 tcr-pull——这是你迁移中的事,未动
...
- [x] **Mac 开发机经 Pangolin 的 `pangolin` 策略 + `make dev` 跑通**(09-12):Mac 不在机房 LAN、`10.10.31.x` 不可达,split DNS 只解决解析。改走现有 Pangolin:云防火墙开 30005、VPS gerbil/Traefik 加 `tcp-30005`(重建约 10 秒中断,已验证恢复)、API 建资源 `consul-dev`(HTTP,SSO 关,tlsServerName/Host=consul.dev.test → .240:443)与 `redis-dev`(raw TCP 30005 → .243:6380 直通);Dragonfly 证书 SAN 加 `redis-dev.apikv.com`(`${REMOTE_HOST}`),install.sh 证书变了也滚动。契约新增 `REMOTE_HOST/PORT/SCHEME/CA`,harvest 新增 `--strategy pre|gateway|pangolin`(dev 默认 pangolin)。dev 环境重写(v6/v7);Mac 上 `cart make dev`:Config Center → PG → **Redis `redis-dev.apikv.com` 连接成功** → 30006 起、healthz 200。Consul 走 `consul-dev.apikv.com` curl 200(make dev 的 CONSUL_ENABLED=false 不注册)。Pangolin 凭据文件与 cookie 用后即删
