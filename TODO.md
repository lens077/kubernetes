...
- [x] **dev 策略真集群实测**(09-12):`ENV=dev` dry-run 对 cart/search 各 9 处改动(redis→`redis.dev.test:6380`、consul→`consul.dev.test:443` https+CA、PG IP→域名),读走 `ecommerce-config-source-dev` 的 service token;在 node4 宿主机(LAN)按这两个地址真握手:Consul 200 + 链过 `global-root-ca`,Redis TLS + AUTH +OK/PING +PONG/错密码 WRONGPASS。**发现**:Mac 不在机房 LAN,VIP 不可达——dev 策略服务的是 LAN 上的开发机;Config Center `dev` 环境现存的是已消亡的内网集群的值(Redis 密码是旧集群的),未重写(需 `ENVIRONMENT=dev` operator token,且要先确认有 LAN 开发机在用)
