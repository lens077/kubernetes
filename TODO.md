...
- [x] ecommerce 搜索已切到 Elasticsearch 后结束回滚窗口，完整卸载 `meilisearch` Helm release，并删除 StatefulSet、Service、ServiceAccount、ConfigMap、master-key Secret、HTTPRoute、10 GiB PVC/PV 与 `search` namespace。
- [x] 安装器保留回滚能力但默认关闭：`bootstrap/config.env`、`bootstrap/config.hosting.env` 的 `ADDON_MEILISEARCH=false`，组件契约 `DEFAULT_ENABLED=false`；安装脚本对旧 `components.selected` fail-closed，node101 的残留选择已删除，其他已有状态须执行 `bootstrap/start.sh --reset-state 80-components` 删除旧选择，再重跑该阶段。只有明确人工回滚时才显式启用并重建索引。
...
- **08-20「ES 已退役、搜索定稿 Meilisearch」结论已被后续决策覆盖**：当前 ecommerce 搜索已切到 node3 Elasticsearch，Meilisearch 运行资源于 2026-09-04 完整退役；本仓 `components/meilisearch` 仅保留默认关闭的人工回滚安装能力
