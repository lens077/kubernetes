...
comp_load_meta() {  # comp_load_meta <组件目录>
  ...
  REMOTE_HOST="" REMOTE_PORT="" REMOTE_SCHEME="" REMOTE_CA=public REMOTE_POLICY=""
...
contract_json() {  # contract_json → 单行 JSON
  ...
  jq -nc --arg id "$ID" --arg provides "$PROVIDES" --arg external "$EXTERNAL" \
    ...
    --arg rh "$REMOTE_HOST" --arg rp "$REMOTE_PORT" --arg rs "$REMOTE_SCHEME" --arg rca "$REMOTE_CA" --arg rpol "$REMOTE_POLICY" \
