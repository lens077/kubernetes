...
已有安装器状态的主机可能在 `$STATE_DIR/components.selected` 留有旧选择；拉取本次退役变更后，先运行 `bootstrap/start.sh --reset-state 80-components` 删除旧选择，再重跑 `80-components` 阶段，让 `ADDON_MEILISEARCH=false` 重新生成选择。安装脚本本身也会 fail-closed：只有把配置显式改为 `ADDON_MEILISEARCH=true` 并重置阶段，或单独执行时显式传入 `MEILISEARCH_RETIREMENT_ROLLBACK=true`，才允许安装。
