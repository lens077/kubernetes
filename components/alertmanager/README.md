...
通知策略（2026-09-26）：`group_wait 1m`、`group_interval 5m`、`repeat_interval 5m`。后者是给桥的刷新，不是手机频率；桥在 PVC 持久化 page 的 1/2/4/8/24h、ticket 的 4/8/16/24h 退避。**先部署新桥与三主题，再部署此路由；不能将 5m 刷新接到旧桥。**
...
- 缺失标签按空字符串参与 `equal`，双方都缺可能误抑制。因此配置要求 cluster/node 非空再相等；没有 node 标签的 Pod 规则不猜测节点归属。
...
| 固定手机重复间隔 | AM 5m 刷新，桥持久指数退避 | 未变化的故障不刷屏，成员变化与新 episode 不受旧冷却误抑制 |
...
# 显式 test 分类，不发核心主题；未配置三主题时新桥返回失败，不假装成功
kubectl -n observability exec statefulset/alertmanager -- wget -qO- --post-data \
  '[{"labels":{"alertname":"ManualTest","severity":"warning","notification_class":"test"},"annotations":{"summary":"手工测试"}}]' \
